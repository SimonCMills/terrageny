...
constructTerrageny <- function(inPath, outPath, inFileStart, fileType, overwrite=F) {
  ...
  if (length(outFiles)!=0) {
    if (overwrite == TRUE) {
      ...
      print("outPath directory overwritten")
    } else {
  ...
  print(paste('Year counter:', all.yrs[t.counter]))
  ...
  final.t <- all.yrs[1]
  print(paste("final dataset year:", final.t))
  ...
  save(file=paste(outPath, '/Results.RData', sep=""), terra.1, terra.2)
  
  #h<-terra.2[terra.2$end.t==0,] 
}